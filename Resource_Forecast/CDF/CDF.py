...
def output_predictions(predictors,true,limit):
    ...
    for predictor in predictors.keys():
        ...
        output["violation_severity"]= np.where(np.array(true)>np.array(pred_vals),((np.array(true)-np.array(pred_vals))/np.array(true)),0).sum()
...
def draw_cdf(data,file_name):
    for key in data.keys():
        ...
        pdf = count / sum(count)
        cdf = np.cumsum(pdf)
        plt.plot(bins_count[1:], cdf, label=key)
